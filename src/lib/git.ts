...
function buildBranchComparison(cwd: string, baseRef: string): { mergeBase: string; commitRange: string } {
...
function formatUntrackedFile(cwd: string, relativePath: string): string {
  ...
  let stat;
  try { stat = statSync(absolute); } catch { return `### ${relativePath}\n(skipped: unreadable)`; }
  if (stat.isDirectory()) return `### ${relativePath}\n(skipped: directory)`;
  if (stat.size > MAX_UNTRACKED_BYTES) return `### ${relativePath}\n(skipped: ${stat.size} bytes exceeds ${MAX_UNTRACKED_BYTES} byte limit)`;
  let buffer: Buffer;
  try { buffer = readFileSync(absolute); } catch { return `### ${relativePath}\n(skipped: unreadable)`; }
...
function collectWorkingTreeContext(cwd: string, state: WorkingTreeState, includeDiff: boolean, truncatedDiffBytes: number): CollectedDetails {
...
function collectBranchContext(cwd: string, baseRef: string, comparison: { mergeBase: string; commitRange: string }, includeDiff: boolean, truncatedDiffBytes: number): CollectedDetails {
...
export function collectReviewContext(cwd: string, target: ReviewTarget, options: CollectContextOptions = {}): ReviewContext {
