...
- **Spec vs Plan vs Backlog vs Milestone vs Research** (keep them separate; non-trivial work has spec before plan):
  ...
  - Naming `YYYY-MM-DD-<topic>-<design|plan|followup|backlog|milestone|research>.md`. **Specs, backlogs, and research accumulate** as long-term records — never archived. **Plans and milestones archive on completion** → `.local/plans/archived/` / `.local/milestones/archived/`.
