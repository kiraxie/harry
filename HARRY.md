...
- **Memory timing:** `CLAUDE.local.md` (per project dir, gitignored) holds only **ACTIVE** state — in-flight work, task lists, who/when/branch. It is loaded every session, so keep it to what is CURRENT: when a session takes over an active item, note it there to avoid multi-session collision; update at commit time, not batched at merge. On **completion**, do NOT grow `CLAUDE.local.md` — append a one-line conclusion to `.local/history.md` (`date · topic · squash SHA · PR# · one-liner`; an on-demand archive, gitignored, NOT auto-loaded) and delete the now-done item from `CLAUDE.local.md`. The detail already lives in git/PR — `history.md` is a thin pointer (don't paste review/lean detail or commit lists into it).
