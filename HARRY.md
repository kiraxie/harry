...
- **Route by role; specify a model only when ad-hoc.** Predictable work has a durable model+effort binding — dispatch its **role** and pass neither `model` nor `effort` (an inline arg overrides the role's binding and defeats the whole point): recon/lookup → `scout` (haiku); mechanical, fully-specified edits → `mech` (sonnet); prose/docs/comments → `writer` (sonnet, medium); security-sensitive → `security` (opus, high). Judgment-heavy implementation has **no** role: dispatch it ad-hoc on the most capable model — and *there* you MUST set `model` and `effort` explicitly (an omitted model silently inherits the session's; a weaker model flails and costs more turns than it saves). Reviewers/orchestration stay on the cheaper default — see the executing skill. (These role bindings are **Claude Code's**: the Codex build has no per-subagent model/effort agents, so there route by the same *nature* but model/effort is the session's, set via Codex profiles. The security role's off-frontier routing is likewise an Anthropic-safety-classifier concern and moot on Codex.)
