...
- **Route by role; specify a model only when ad-hoc.** Predictable work has a durable model+effort binding — dispatch its **role** and pass neither `model` nor `effort` (an inline arg overrides the role's binding and defeats the whole point): recon/lookup → `harry-scout` (haiku); mechanical, fully-specified edits → `harry-mech` (sonnet); prose/docs/comments → `harry-writer` (sonnet, medium); security-sensitive → `harry-security` (opus, high). Judgment-heavy implementation has **no** role: dispatch it ad-hoc on the most capable model — and *there* you MUST set `model` and `effort` explicitly (an omitted model silently inherits the session's; a weaker model flails and costs more turns than it saves). Reviewers/orchestration stay on the cheaper default — see the executing skill. (The security role runs off the frontier tier because a frontier safety classifier can refuse benign defensive work mid-task — Anthropic-specific, moot on the Codex build.)
