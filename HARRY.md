...
- **Spec vs Plan vs Backlog** (keep them separate; non-trivial work has spec before plan):
  ...
  - Naming `YYYY-MM-DD-<topic>-<design|plan|followup|backlog>.md`. **Specs and backlogs accumulate** as long-term records — never archived. Only **plans** archive after execution → `.local/plans/archived/`.
  ...
  - **Backlog vs `/debt`**: `/debt` re-judges deferred *decisions* (DEBT: markers, spec Non-Goals, plan deferrals) — each has a premise that can go stale, and re-judging that premise is the whole point. Backlog holds *undecided* items — there is no premise to re-judge, only the open question "is this worth deciding now." `/debt` does not scan `.local/backlog/`.
