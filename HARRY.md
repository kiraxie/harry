...
- **Running systems: state the blast radius before touching them.** Before any command that stops, disables, deletes, or reconfigures a running service: state the exact command, what breaks if it is wrong, the exact rollback command, and whether a replacement is registered and verified healthy — then wait for the go-ahead. Read-only diagnostics need none of this.
- **Secrets stay out of the transcript.** Reading dotfiles, `.env`, shell rc files, or anything that may hold credentials: print variable names only, or mask the values (`sed 's/=.*/=***/'`). A key that lands in a transcript is a key to rotate — the read itself is the leak, not the later paste.
