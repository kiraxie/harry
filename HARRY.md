...
- **Optimize on evidence, never on imagination.** Optimization spends clarity; the receipt is a measurement. No cache, index, memoization, micro-optimization, or clever rewrite without a profile/benchmark naming *this* path on a *real* workload — "should be faster" is a banned claim (§6). The default is the simple version — with a `DEBT:` ceiling (§4) when it has one worth naming; upgrade when N actually approaches it. This is YAGNI on the performance axis — speculative optimization is the same mistake as speculative abstraction. Three things it does NOT ban: design-time scale decisions (data model, asymptotic class, round-trip/query shape against a *known* workload — those are requirements settled cheaply up front, not optimizations); an explicit perf budget or SLO (a requested behavior — §2); and avoiding gratuitous pessimization — when the equally clear idiom is also the faster one (the right data structure, no N+1, no needless copy in a loop) pick it, that is not optimizing but declining to waste.
...
- **Route by role; specify a model only when ad-hoc.** Predictable work has a durable model+effort binding — dispatch its **role** and pass neither `model` nor `effort` (an inline arg overrides the role's binding and defeats the whole point): recon/lookup → `scout` (haiku) — this includes read-only git inspection (`log`/`diff`/`status`/`show`/`blame`) and grep/find-type searches when dispatched to a subagent; mechanical, fully-specified edits → `mech` (sonnet); prose/docs/comments → `writer` (sonnet, medium); security-sensitive → `security` (opus, high). Judgment-heavy implementation has **no** role: dispatch it ad-hoc on the most capable model — and *there* you MUST set `model` and `effort` explicitly (an omitted model silently inherits the session's; a weaker model flails and costs more turns than it saves). Reviewers/orchestration stay on the cheaper default — see the executing skill. (These role bindings are **Claude Code's**: the Codex build has no per-subagent model/effort agents, so there route by the same *nature* but model/effort is the session's, set via Codex profiles. The security role's off-frontier routing is likewise an Anthropic-safety-classifier concern and moot on Codex.)
- **`.local/` doc types + `INDEX.md`** (keep them separate; Major work has a spec before a plan; Standard records decisions in a spec only when real alternatives were weighed): a work unit is one **item** (`.local/items/<slug>.md`) that moves through `status: backlog → active → done` in place — `active` holds a `## Why / What` (what a spec used to hold) and a `## Plan` (what a plan used to hold) in the same file. Undecided ideas stay at `status: backlog` (absorbs what used to be a separate research doc); a mid-size goal spanning several items gets a `type: milestone` item that links to its members, not a new folder. On completion the item moves to `.local/archive/`, read-only. `.local/INDEX.md` is the live content map over active/backlog items. The full item format — frontmatter schema, milestone References/Members/Delivered semantics, lifecycle, `/debt` boundaries, INDEX update rules, and the lazy-derivation rule for "current state" — lives in **`references/doc-types.md`; read it whenever you create, graduate, or archive any `.local/` item.** (The pipeline skills already do their item/INDEX steps inline.) These laws load globally, so a project may never have run `/harry:sync`: if `.local/` doesn't exist, set the project up (or confirm with the user) before creating `.local/` artifacts there — don't scatter untracked docs into a repo whose `.gitignore` lacks harry's block.
...
- **Memory timing:** Active in-flight work lives in the `## In flight` section at the top of `.local/INDEX.md` — one line per active unit (topic, branch, started date). It is gitignored via `.local/` and **NOT auto-loaded** (lazy): knowing what to work on and not duplicating an in-flight item is the user's call at session start; harry's job is only to keep the list accurate. **When you begin a unit of work, mark it started in `.local/INDEX.md` `## In flight`** (topic, branch, date; wired into the executing skill); update at commit time, not batched at merge. On **completion**, append a one-line conclusion to `.local/HISTORY.md` (`date · topic · squash SHA · PR# · one-liner`; an on-demand archive, gitignored, NOT auto-loaded — rotate yearly per `references/doc-types.md`) and remove the unit's In-flight line (wired into finishing). The detail already lives in git/PR — `HISTORY.md` is a thin pointer (don't paste review/audit detail or commit lists into it). (`CLAUDE.local.md` is separate: per-project specialization rules, gitignored, loaded every session — it is NOT the work list.)
