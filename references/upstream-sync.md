...
- The resident laws (`HARRY.md`) track `ponytail`, still recalibrated (correctness > cost) — most upstream "be lazier" changes do NOT apply; read the `note`. Their other origin, `superpowers`, is retired — re-compare on demand only.
- **`grill`'s upstream is `mattpocock-skills`** (the grill family: grilling primitive, grill-me, batch-grill-me). Harry **shares** upstream's one-question-at-a-time cadence canon (its grilling primitive, line 8 of `SKILL.md` under `skills/productivity/grilling` in that upstream) — don't "restore" it as if it were a deviation, there is nothing to restore. The harry-authored parts, with NO upstream counterpart, are the residue-manifest exit gate, the assumption dispositions (confirmed fact / returned as an open question / pinned by an AC), and the ledger (decided/open/deferred/assumptions) it reads out from. The same survey also fed smaller distills into `root-cause-tracing.md`, `skill-authoring.md`, `red-green.md`, and brainstorming (see `derived[]`).
