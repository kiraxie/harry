...
Whether the main checkout is on `<branch>` right now is not part of it either; only Discard needs that, and checks it itself.
...
- **Follow-up commits re-run step 2 before they are pushed.** A commit answering PR feedback can alter a shape: run step 2's shape gate as a later round — shapes changed since the head the most recent round recorded — and review any it finds before the push.
