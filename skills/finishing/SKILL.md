...
d. **Milestone membership** — if the item's frontmatter has `milestone: <slug>`, open that milestone item (`.local/items/<slug>.md`) and move this item's link from its `## Members` to its `## Delivered` section. If `## Members` is now empty (this was the last one), archive the milestone itself right here, same as step c: `status: done`, move it to `.local/archive/<slug>.md`.
