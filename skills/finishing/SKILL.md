...
- **On merge** — whether in this session or a later one, and whether you merged it or a human clicked merge on GitHub — finishing resumes: run Option 1's full tail a–h, EXCEPT the merge itself (already done). Step a there is a local re-verify of the merged base; evidence step h uses the CI run the push/PR already triggered. If you notice a merged PR whose item is still `status: active`, that's the trigger to run the tail now.
