...
f. **Cleanup** — by this step you have already left the worktree (step a ran on `<base>`), so never derive its path from cwd: look it up with `git worktree list` filtered for `<branch>`. Remove the worktree, then delete the feature branch, in this order:
   ...
   Provenance rule: only clean up worktrees YOU created — never remove harness-owned ones. **If removal is refused** — native tooling declining without a discard flag, or `git worktree remove` reporting "contains modified or untracked files" — those files exist nowhere else. Never force the removal on your own initiative (no `--force`, no discard flag): show the user `git -C <looked-up-path> status --porcelain -uall` and ask which:
   ...
   Carry out the choice, then remove the worktree. Also delete this unit's `.local/tmp/<branch>/` (briefs/reports/diffs are transient; orphaned per-branch dirs otherwise accumulate in the main checkout forever) — never touch sibling branches' dirs.
...
Never offered from the menu; run this only when the user asks to discard the work. Show what will be lost (branch, commit list, worktree path, and the worktree's uncommitted/untracked files via `status --porcelain -uall`), then require a typed `discard` to confirm — given after the user has seen that listed-files loss, this typed confirmation is what authorizes forcing the worktree removal here, unlike step f's refused-removal flow above, which never forces on its own initiative. On confirmation: `cd` to main root, clean up the worktree (provenance rule), delete `.local/tmp/<branch>/`, then `git branch -D <branch>`.
...
|--------|:--:|:--:|:--:|:--:|--------|----------|
| 1. Merge | green required ×2 (branch, then merged result) | before the merge, when a shape changed | squash | — | force-deleted after merged result is green AND the landing check passes; a `<branch>-rescue` may survive a refused removal | removed first, only when clean (native tooling, provenance) |
| 2. PR | green required | before `gh pr create` and each follow-up push, when a shape changed | — | yes (draft approved first) | kept | **kept** (needed for iteration)¹ |
