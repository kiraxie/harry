...
Parallel units' worktrees are not part of it: executing removes each as soon as its task merges back. Whether the main checkout is on `<branch>` right now is not part of it either; only Discard needs that, and checks it itself.
...
f. **Cleanup** — by the unit's layout (Where the unit lives): **in place** runs only f.1, f.4 and f.5; **own worktree** runs f.1–f.5, using the worktree path the layout recorded. Remove the worktree, then delete the feature branch, in this order:
   1. **Landing check.** A squashed branch is never an ancestor of `<base>`, so `git branch -d` refuses it and native worktree tooling reports its commits as unmerged. Prove `<base>` already contains the branch's whole change: `test "$(git merge-tree --write-tree <base> <branch>)" = "$(git rev-parse '<base>^{tree}')"` (git ≥ 2.38) — merging the branch into `<base>` would change nothing. Compare the command's whole output, never just its first line: on a conflict it still prints a tree first, and a modify/delete or binary conflict can leave that tree equal to `<base>`'s. It holds for a squash, a merge commit, a rebase merge, and a base that already carried part of the change. Not equal → STOP: leave branch and worktree, show the user `git diff <base> <branch> --stat`; something on the branch did not land. For a PR, run it against `origin/<branch>` after `git fetch`, and confirm the local branch has no commits the remote lacks; if the remote branch is already gone (deleted on merge), run it against the local `<branch>` and compare its tip with the PR head (`gh pr view <n> --json headRefOid`).
   2. **Clean worktree.** `git -C <looked-up-path> status --porcelain -uall` must be empty; if not, run the refused-removal flow below before anything is forced. Also list ignored files — `git -C <looked-up-path> ls-files -o -i --exclude-standard` — and show them to the user before removal: `-uall` alone does not list them, removal drops them, and no rescue choice below covers them (usually dependency/build output, but the user says whether anything there matters).
   3. **Remove the worktree** — prefer the harness's NATIVE worktree tooling; its discard flag is allowed only when f.1 passed AND f.2 found the worktree clean (the flag also destroys uncommitted files); only as fallback, from the main repo root: `git worktree remove <looked-up-path>` and `git worktree prune`.
   4. **Delete the branch** — `git branch -D <branch>` (git refuses to delete a branch still checked out in a worktree, which is why this comes last).
   5. **Temp files.** Delete this unit's `.local/tmp/<branch>/` (briefs/reports/diffs are transient; orphaned per-branch dirs otherwise accumulate in the main checkout forever) — never touch sibling branches' dirs. A branch name with `/` (`chore/x`) leaves its parent (`.local/tmp/chore/`) behind: remove it too when it is now empty (`rmdir`, which refuses a non-empty one). This runs whether or not the unit had a worktree.

   Provenance rule: remove only the worktree the layout names as the unit's own (and, in Discard, the ones the user named) — never any other worktree, whoever created it. **If removal is refused** — native tooling declining without a discard flag, or `git worktree remove` reporting "contains modified or untracked files" — those files exist nowhere else. Never force the removal on your own initiative (no `--force`, no discard flag): show the user `git -C <looked-up-path> status --porcelain -uall` and ask which:
...
- **Follow-up commits re-run step 2 before they are pushed.** A commit answering PR feedback can alter a shape too: run step 2's shape gate as a later round — shapes changed since the head the most recent round recorded — and review any it finds before the push.
