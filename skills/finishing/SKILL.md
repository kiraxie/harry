...
f. **Cleanup** — by this step you have already left the worktree (step a ran on `<base>`), so never derive its path from cwd: look it up with `git worktree list` filtered for `<branch>`. Remove the worktree, then delete the feature branch, in this order:
   ...
   2. **Clean worktree.** `git -C <looked-up-path> status --porcelain -uall` must be empty; if not, run the refused-removal flow below before anything is forced.
   3. **Remove the worktree** — prefer the harness's NATIVE worktree tooling; its discard flag is allowed only when step 1 passed AND step 2 found the worktree clean (the flag also destroys uncommitted files); only as fallback, from the main repo root: `git worktree remove <looked-up-path>` and `git worktree prune`.
   ...
   Provenance rule: only clean up worktrees YOU created — never remove harness-owned ones. **If removal is refused** — native tooling declining without a discard flag, or `git worktree remove` reporting "contains modified or untracked files" — those files exist nowhere else. Never force the removal on your own initiative (no `--force`, no discard flag): show the user `git -C <looked-up-path> status --porcelain -uall` and ask which: 1. commit them to a new branch cut from `<base>` (e.g. `<branch>-rescue`) and keep that branch for the user to integrate through the normal merge-or-PR flow — it is not deleted by this cleanup; 2. move them into the main checkout; 3. delete them (unrecoverable). Carry out the choice, then remove the worktree. Also delete this unit's `.local/tmp/<branch>/` (briefs/reports/diffs are transient; orphaned per-branch dirs otherwise accumulate in the main checkout forever) — never touch sibling branches' dirs.
