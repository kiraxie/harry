...
Implementer = **CC's own subagents** (not an external delegate). Each gets isolated, precisely-built context — never your session history — via **file handoffs**, plus an explicit output contract. A task is one AC, or a group of AC that must land together. Tasks with data/interface dependencies run sequentially; independent tasks default to parallel dispatch, each in its **own** worktree (HARRY.md §5), cut from the unit's branch — if the harness cuts isolation worktrees from the main checkout's HEAD instead, the brief's first step is `git reset --hard <unit branch>`, or the unit builds on a stale base. For each parallel task, branch a per-task worktree off the unit branch; the task stays there through its review and fix rounds. Merge each back into the unit branch once step 5 marks it complete, resolve conflicts, then run the suite on the unit branch. Then remove that task's worktree and delete its branch right away — the merge just proved it landed: `git -C <path> status --porcelain -uall` must be empty first (if not, stop and ask), then `git worktree remove <path>` and `git branch -D <its branch>`. A parallel worktree never outlives the task that made it; finishing does not look for any.
...
4. **Fix loop — capped at four rounds.** The cap operationalizes §6's three-failed-fixes law: three failures of the same fixer mean the fourth round changes the *fixer*, not the hypothesis; still open after that is structural, so adjudicate instead of looping. **The session is never the fixer here** — fixing a finding yourself skips the loop's re-review; every fix goes through the loop's fixer below (the resumed implementer in rounds 1–3, the fresh one in round 4). A parallel task's fixer — resumed or fresh — works in that task's worktree, which is still there: it is removed only after step 5. Give a fresh fixer that worktree's absolute path; never dispatch it with a new isolation worktree, which would be cut from elsewhere and hold none of the task's commits.
