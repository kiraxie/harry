...
**Model by role — always specify it; an omitted model silently inherits the session's.** Implementer and fixer default to the **most capable available model — currently `opus`** (update this literal default here the day a stronger model ships; do not leave it as a vague "figure it out at dispatch time" — that phrasing was tried and, in practice, made it too easy to silently fall back to the session's own model instead): the role does judgment/exploration, and a weaker model flails and burns more turns than it saves (turn count beats token price). Route to a **cheaper, faster model** (`sonnet`) ONLY when the task's *nature* is mechanical/transcription — the plan already carries the complete code, or it's a single-file rote change with no design decision to make. The task's *nature* decides this, never a blanket label — a mechanical Major task (e.g. the same field added across 8 CRUD files) still routes cheap; a subtle one (e.g. a 2-file concurrency fix) still routes capable. Reviewers are routed separately (step 3).
