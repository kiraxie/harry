...
function brandMarkup() {
  // harry: neutral text-only brand — no external image fetch (personal plugin
  // must not phone home for assets). Version reads harry's own package.json.
  const version = escapeHtmlText(SUPERPOWERS_VERSION);
  return '<div class="brand"><span class="brand-copy">harry · Visual Companion v' + version + '</span></div>';
}
