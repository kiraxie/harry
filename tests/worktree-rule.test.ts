...
const has = (text: string, literal: string): void =>
  assert.ok(text.includes(literal), `missing: ${literal}`);
...
test("HARRY.md §5 ties worktree isolation to concurrent writers, not tier", () => {
  ...
  // The law states only the invariant; how a task worktree is cut and recorded
  // lives in executing, so the two cannot drift apart on the mechanism.
  assert.match(
    section,
...
test("parallel worktrees are removed by executing, in step 5's integration", () => {
  ...
  // AC-31(c): 5.5 deletes the task branch only when it holds no work the unit
  // branch lacks. "The fast-forward just proved it landed" held only in an
  // uninterrupted run, not on resume. The proof is a tree-equality check, stated
  // in 5.5 and cited by finishing's f.1: it counts content, so an empty task's `--no-ff` merge
  // passes and a conflict resolution kept only in a merge commit does not.
  has(
    executing,
  ...
  assert.doesNotMatch(executing, /rev-list --count --no-merges <unit branch>\.\./);
...
test("Discard finds the unit's task worktrees by executing's naming rule, and the user confirms", () => {
  const discard = read("skills/finishing/SKILL.md").split("### Discard")[1] ?? "";
  // Asking the user to pick the unit's worktrees out of every linked one made them
  // guess; executing's naming rule identifies them, and the user confirms the list.
  assert.doesNotMatch(discard, /have the user name which are this unit's/);
  ...
  // Only confirmed tasks lose their branch and base ref; the whole prefix goes
  // only when the user confirmed every listed entry.
  assert.match(
    discard,
  ...
  // Deleting the branch alone would leave a base ref that reads as a unit-branch task.
  // The transaction is stated once, at executing's 5.5, and cited here.
  has(discard, "executing's 5.5 transaction, `<branch>` standing for `<unit branch>`");
  ...
  // A partial confirmation keeps the unit branch, so the unconfirmed tasks'
  // branches and refs never outlive the branch whose name they carry.
  assert.match(discard, /When the user leaves any listed entry unconfirmed, keep `<branch>` too/);
...
test("finishing's cleanup removes any task a run left behind, branch and base ref together", () => {
  // executing's step 5 removes each task once it lands, so a leftover one is a task a
  // run stopped short of removing; its branch left alone would sit in step 2's way
  // for a later unit that reuses the unit branch's name.
  const finishing = read("skills/finishing/SKILL.md");
  // Only executing's 5.4 writes a complete line, and a return to executing must come
  // before anything merges, archives the item or removes the unit's worktree, so the
  // classification is stated in step 1, ahead of the menu and of f.4.
  ...
  // AC-28: the leftover-task classification is stated once, in executing's
  // Resuming table. Finishing runs it and acts only on its run-5.5 row; a second
  // case list here had already drifted from executing's (it guarded deletion with
  // a containment check executing lacked).
  has(
    step1,
  ...
  // f.2 re-runs step 1's classification from git and the archived Progress right
  // before removing anything: Option 2's on-merge resume reaches it without step 1.
  const leftoverStep =
    finishing.split("2. **Leftover tasks.**")[1]?.split("3. **Clean worktree.**")[0] ?? "";
  has(leftoverStep, "`.local/archive/<slug>.md`");
...
test("the report log is evidence: resume routes on git and `## Progress` only (AC-26)", () => {
  ...
  // Routing on the log made it a state machine whose every entry had to be exact;
  // each fix to its ordering, closing, `file:` and `head:` rules opened the next
  // window. Nothing routes on it now, so those rules are gone.
  has(executing, "The log is evidence, not state");
  ...
  // AC-31(a): whether an agent is still running cannot be read from git or
  // `## Progress`; with no automatic re-dispatch, no rule needs to know. The
  // removed rule's own words: "the session waits for its completion notification".
  assert.doesNotMatch(executing, /the session waits/);
  ...
  // One review file per attempt stays: a reused name let a stale report pass as the
  // live one. Its number comes from the files, not from counting log entries.
  has(executing, "`<store>/.local/tmp/<branch>/<task id>-review-<k>.md`");
  ...
  assert.doesNotMatch(executing, /still running: wait for it/);
  ...
  // AC-31(a): no automatic re-dispatch. A task with no commits and a clean tree
  // may have an agent still working on it, or may have returned BLOCKED; neither
  // is visible in git or `## Progress`, so the user rules.
  assert.doesNotMatch(step5, /re-dispatch it, reusing/);
  // The rest of the table is pinned in the AC-33 tests.
  assert.doesNotMatch(executing, /no rule here waits/);
  assert.doesNotMatch(executing, /one automatic resume/);
  has(step5, "its last `## Progress` lines, the log's last entries");
  // A scoped re-review after a resume has no recorded start: the base ref, unless
  // the user names a head.
  has(
    executing,
...
test("a task the user sends back from Resuming has one round name per destination (AC-32 m-a)", () => {
  const executing = read("skills/executing/SKILL.md");
  // Back to step 2, to implement again → `initial`; back to step 4 with findings
  // → one past the last `fix round` line, `1/4` when there is none. Two rules for
  // one case gave a restarted task two labels and moved round 4's escalation.
  has(
    executing,
...
test("the Codex build's log is the session's own (AC-29)", () => {
  ...
  has(
    carveOut,
    "With no subagents on that build, the session implements each task itself and appends every log entry, the `## result` ones included, and it is step 4's fixer too: every fix still goes through that step's re-review, and round 4's escalation is a switch to the role map's most-capable row",
  );
  // AC-31(d): step 4's rule names the carve-out instead of contradicting it.
  has(
    executing,
    "**The session is never the fixer here** (on the Codex build it is: see Model by role)",
  );
});
...
test("a parallel task integrates in its own worktree before it is marked complete", () => {
  ...
  // Step 5 owns them; step 4 and the intro only point here (one copy, no drift).
  assert.match(step5, /a fix that needs another unit's files is BLOCKED/);
  assert.match(
    step5,
  ...
  assert.match(executing, /Step 5's integration findings enter this loop under that step's limits/);
});
...
test("step 5 writes the complete line before removal, and asks rather than guesses on resume", () => {
  ...
  // Inferring where a crashed task stopped kept leaving windows that landed
  // unreviewed code or orphaned worktrees; the user decides instead.
  assert.match(step5, /is not resumed by inference/);
  assert.match(step5, /Stop and ask the user, showing its branch/);
  // A resumed session must look for surviving worktrees, and step 4's own resume
  // lines must defer to this rule rather than contradict it.
  assert.match(step5, /before it dispatches anything, it reads the unit's task worktrees from git/);
  assert.match(
    step5,
    /a worktree, branch or ref outside those prefixes is not this unit's: leave it untouched/,
  );
  // A harness isolation worktree is named only when the agent returns, and not
  // by the naming rule, so git would not identify it as the unit's.
  assert.match(executing, /never a harness isolation worktree/);
  ...
  assert.doesNotMatch(executing, /paragraph handles/);
  ...
  assert.match(step5, /`git log --first-parent <base7>\.\.<head7>` lists the task's own commits/);
  has(
    step5,
...
test("git is the ledger for task worktrees: naming, base ref, --no-ff, idempotent removal", () => {
  // `## Progress` lines about worktrees could be lost or wrong after a crash; git
  // state cannot disagree with itself, so the name and a ref carry the record.
  const executing = read("skills/executing/SKILL.md");
  const step5 =
    executing.split("5. **Integrate, then mark complete.**")[1]?.split("After all tasks:")[0] ?? "";
  assert.match(executing, /\*\*task id\*\* — its AC ids joined with `-`/);
  // Brief, report and diff are named by task id, so after compaction or in a new
  // session git's task id locates them; a session-local ordinal did not.
  assert.match(executing, /`<store>\/\.local\/tmp\/<branch>\/<task id>-brief\.md`/);
  ...
  // Branch and base ref are created in one transaction, before the worktree: a task
  // with its own worktree has both or neither, so a base ref without a task branch
  // is a unit-branch task by construction (a lone base ref was misread after a
  // failed `worktree add` while another task landed).
  assert.match(executing, /git update-ref --stdin/);
  ...
  // Create-only base ref: an existing one is never moved, and its refusal is the
  // resume signal (an overwritten base made Resuming record a false completion).
  assert.doesNotMatch(executing, /base "\$base" "" && git worktree add/);
  assert.match(
    executing,
    /git update-ref refs\/harry\/<unit branch>\/<task id>\/base <unit branch> ""`/,
  );
  ...
  // `create` refusing is the signal that git already holds the task. Any resumed
  // session, a compacted one included, has already run Resuming on it, so a
  // refusal is unexpected: ask.
  assert.match(
    executing,
  ...
  // The report file is an append-only log, but only evidence (AC-26): its
  // ordering and closing rules are pinned absent in the AC-26 test above. A log
  // with no entry says nothing about whether a task was dispatched.
  assert.doesNotMatch(executing, /never dispatched: dispatch it/);
  ...
  // A re-dispatch (Resuming's nothing-done row, NEEDS_CONTEXT) reuses what the
  // first start wrote; only the first start runs the create-only writes.
  has(
    executing,
  ...
  assert.doesNotMatch(executing, /is in flight, and/);
  ...
  assert.doesNotMatch(executing, /finishing does not look for any/);
  assert.match(
    executing,
  ...
  // Any resumed session runs Resuming, a compacted one included (AC-23): with no
  // wait and no repair rows left, Resuming is safe after compaction.
  assert.doesNotMatch(executing, /\(restart\/compaction\)/);
  assert.doesNotMatch(executing, /Compaction is not a restart/);
  assert.doesNotMatch(step5, /never after compaction/);
  // The superseded Progress records are gone.
  assert.doesNotMatch(executing, /dispatched on/);
  ...
  assert.match(step5, /so 5\.5 is safe to re-run/);
  // A registered worktree whose directory is gone still blocks `git branch -D`;
  // `git worktree remove` clears only that registration, where a prune would drop
  // every stale registration in the repo, the unit's or not.
  assert.match(step5, /if `git worktree list` lists `<path>`/);
  assert.match(
    step5,
  ...
  // A task with nothing to land writes its complete line with no fast-forward. Its
  // own commits exclude the `--no-ff` integration merge, which would otherwise make
  // an empty task land other tasks' work as its own.
  assert.match(step5, /has nothing to land/);
  ...
  // A leftover base ref is never deleted before its complete line: it is the
  // branch point the complete line's range starts from, so it is reused.
  assert.doesNotMatch(step5, /absent → delete the ref/);
  ...
  assert.match(step5, /delete refs\/heads\/task\/<unit branch>\/<task id>/);
  assert.match(step5, /delete refs\/harry\/<unit branch>\/<task id>\/base/);
  assert.match(step5, /only after the worktree is gone/);
  // Resuming is a table over git state; its rows are pinned in the AC-33 tests.
  // A unit-branch task without a complete line goes to the user (AC-31, AC-33).
  assert.doesNotMatch(step5, /nothing done: remove its worktree/);
...
test("doc-types lists the Progress lines rules read back, and names the three stores (AC-27)", () => {
  ...
  // The report log arrived after the two-store framing and changed it: a reader
  // deciding what is safe to delete, or where resume reads from, needs all three.
  has(
    progress,
...
test("the final review's fixes hold (AC-24)", () => {
  ...
  const step5 =
    executing.split("5. **Integrate, then mark complete.**")[1]?.split("After all tasks:")[0] ?? "";
  // Task worktrees live under the git directory, outside every working tree, so
  // directory-scanning tools (test runners, tsc, watchers) never see them.
  has(executing, "worktree `<git common dir>/harry-worktrees/<unit branch>/<task id>`");
  ...
  // AC-25: a re-review range is named by ref, never read from the session's
  // working directory (the unit checkout, whose HEAD is not a worktree task's tip).
  // AC-26: its start is no longer recorded in the log (pinned in the AC-26 test).
  has(
    executing,
  ...
  has(executing, "into its existing worktree (the unit checkout for a task with no task branch)");
  // The changelog's worktree bullet stays wrapped (read raw: `read` collapses
  // whitespace, so it cannot see a line that was spliced in unwrapped).
  const bullet =
    readFileSync(path.join(repoRoot, "CHANGELOG.md"), "utf-8")
      .split("again by name at the final review.")[1]
      ?.split("\n- ")[0] ?? "";
  assert.ok(bullet.length > 0, "the changelog's worktree bullet moved");
  ...
  // Mid-loop resume by the last `## Progress` line is session mode's; subagent mode
  // resumes through Resuming, from git and `## Progress`.
  const session = executing.split("## Session mode")[1]?.split("## Subagent mode")[0] ?? "";
  has(session, "a unit whose last `## Progress` line is a fix round resumes mid-loop");
  ...
  // One name for the record that marks an AC done.
  assert.doesNotMatch(executing, /done line/);
  has(executing, "An AC with a complete line is DONE");
  // Discard deletes base refs with no complete line; executing never does.
  has(step5, "Executing never deletes a base ref before its task's complete line");
  ...
  const changelog = read("CHANGELOG.md");
  has(changelog, "a resume never re-dispatches a task on its own");
...
// Resuming's table, read line by line from the raw file: the rows are the
// contiguous `|` lines after its header, so no later sentence can shrink the
// slice (AC-33 m-3).
...
test("Resuming continues on its own only in the run-5.5 row (AC-33)", () => {
  ...
  // Two scoped reviews in a row forged the fast-forwarded row (C-1, C-2), each
  // ending in a false, append-only `review clean` complete line. It is gone:
  // every task without a complete line goes to the user.
...
test("5.3 has no fast-forward witness (AC-33)", () => {
  ...
  has(executing, "has nothing to land: skip the fast-forward and go to 5.4");
});
...
test("Resuming has no dirty-tree row: 5.5's own check covers it, MERGE_HEAD included (AC-33)", () => {
  const executing = read("skills/executing/SKILL.md");
  // The only automatic row runs 5.5 (finishing's f.2 runs it too), which already
  // refuses a dirty worktree. A merge in progress can leave `status --porcelain
  // -uall` empty (a clean `--no-commit` merge, or a conflict resolved back to
  // ours), so 5.5's clean check covers MERGE_HEAD too; the check itself is
  // pinned in the AC-34 (c) test.
  for (const row of resumingTable()) assert.doesNotMatch(row, /MERGE_HEAD|uncommitted files/);
  const remove =
    executing
...
test("a check that cannot run counts as not passing, stated once (AC-33 I-B)", () => {
  ...
  // No example that contradicts 5.5, which removes a registration whose
  // directory is gone without running `status` on it (AC-33 I-D).
  assert.doesNotMatch(executing, /cannot run — /);
...
test("finishing removes a leftover task by running executing's 5.5 (AC-33 I-C)", () => {
  ...
  // AC-28: finishing acts on the run-5.5 row by running executing's step, never a
  // restated copy — the copy had drifted (no MERGE_HEAD, no gone-directory guard).
  // The citation itself is pinned in the AC-34 (e) test.
  assert.doesNotMatch(deleteStep, /clean check/);
...
test("the worktree clean check is stated once, in 5.5 (AC-34 c)", () => {
  ...
  has(executing, check);
  // 5.5 says why it lists no ignored files; finishing lists them for the unit's
  // own worktree only, and says why.
  has(
    executing,
...
test("a merge in progress is finished or aborted first, then cleanup restarts from f.1 (AC-34 m-12)", () => {
  ...
  // A commit that finishes the merge lands on the branch after f.1 proved it, so
  // f.1 runs again before f.5's `branch -D`; and git cannot cut a rescue branch
  // mid-merge, so no rescue choice runs while MERGE_HEAD is set.
  const merge =
...
test("the report log's entries are specified once, in the log paragraph (AC-35 a)", () => {
  ...
  for (const entry of [
    ...
    has(log, entry);
  // Outside the log paragraph, only the Codex carve-out names an entry (who
  // appends it on that build); every per-step restatement is gone.
  const carveOut =
    executing.split("**Model by role.**")[1]?.split("**Reviewers on that build:**")[0] ?? "";
  const rest = executing.replace(log, "").replace(carveOut, "");
  assert.doesNotMatch(rest, /`## (dispatched|result|review)/);
...
test("the deletion proof is defined in executing 5.5 and cited by finishing (AC-35 b)", () => {
  ...
  const remove =
    executing
      .split("5. **Remove the task's worktree and delete its branch**")[1]
      ?.split("**Resuming.**")[0] ?? "";
  // Generic operands, then each caller's own binding: "X standing for Y" read
  // both ways, and a review mapped finishing's `<branch>` to a task branch.
  has(
    remove,
