---
description: Ask a single frontier model one prompt (read-only) and print the answer. This is the gpt-5.5 backend used by /debate. Roughly 1 Copilot premium request per call.
...
- Costs ~1 Copilot premium request per call. If `status` is `blocked` (quota) or `failed`, surface the message and stop.
