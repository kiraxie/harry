...
A security audit has a clean anchor: "can you build an exploit?" This domain does not. "This should use the Strategy pattern" is a taste claim, and if you let taste drive findings the whole audit degrades into two agents arguing aesthetics. So this command is built around a substitute anchor — the **falsifiability anchor** below — and everything downstream (the adversarial round, the verification round) exists to enforce it.

## Where this sits among related tools

| Tool | Direction | Scope | Cadence |
|---|---|---|---|
| a diff-level code review (`/review`) | correctness — plus a quick over-engineering pass via `--simplify`'s CC-native Lane B | a change | per PR |
| a debt-ledger pass (`/debt`) | re-judge existing debt markers | existing markers | one-shot |
| **`/audit` (this)** | **restructure / consolidate** (incl. hoist, dimension 2) **and deep over-engineering hunting** (dimension 10) | **whole repo / subsystem** | **iterative, accumulative** |

Over-engineering has two speeds now, not two competing tools: a quick per-diff pass
lives in `/review --simplify`'s CC-native Lane B (see **the simplify dual-lane** in
`commands/review.md`; cheap, no extra Codex quota); the
deep, evidence-verified, severity-ranked version is dimension 10 here. The one place
`/audit`'s hoist dimension (2) and its own over-engineering dimension (10) could seem
to contradict — one says *extract a shared abstraction*, the other says *delete
abstraction* — is reconciled by the **drift test**: dimension 2 only proposes a hoist
when divergence would be a bug (cross-boundary shared truth), which is exactly the
case where even KISS/YAGNI says extract. Outside that case, `/audit` leaves
duplication alone. So the two dimensions never fight over the same code.
...
- **Task tool** means the coding agent's delegation / sub-agent mechanism (the `Agent` tool here).
...
- **Target**: the codebase to audit (from `$ARGUMENTS` or the current working directory). Confirm the scope — whole repo, or a named subsystem / set of workspaces.
- **Output directory**: where all audit artifacts go. If `$ARGUMENTS` has `--output <dir>`, use it. Otherwise, if the repo has a gitignored `.local/` directory, default to `.local/audit/run-<N>/`; else default to `.code-audit/run-<N>/`. `<N>` is the next unused integer (check with `ls`). Ask the user if they want somewhere else. Create it if missing. Separate run directories are what make the audit **iterative** (see below).
...
Like a security audit, one run does not find everything: which agents dig where determines what surfaces. This command is meant to be **run repeatedly**, each run a full pass that accumulates.
...
The natural bias of an architecture reviewer is to *add* abstraction (introduce a Strategy, a Factory, a Repository). That bias directly fights KISS / YAGNI and produces over-engineering — the exact thing dimension 10 (and `/review --simplify`'s quick pass) exists to remove. So this audit holds the opposite default:
...
### Anti-patterns (these make a code audit useless)
...
Follow all six rounds in order. Each round's detail lives in a reference file under `references/audit/` — read it when you reach that round.

1. **Recon & substrate** — [references/audit/RECON.md](${CLAUDE_PLUGIN_ROOT}/references/audit/RECON.md). Run language-appropriate deterministic tools to build the factual substrate; extract the codebase's own `intended-architecture.md`. Includes Stage A of the hoist-candidate funnel (deterministic candidate generation).
2. **Deep-dive hunt** — [references/audit/DEEP-DIVE.md](${CLAUDE_PLUGIN_ROOT}/references/audit/DEEP-DIVE.md), selecting dimensions from [references/audit/SCAN-DIMENSIONS.md](${CLAUDE_PLUGIN_ROOT}/references/audit/SCAN-DIMENSIONS.md). Fan out `general` agents by *workspace × dimension*; each finding must carry the falsifiability anchor. Includes Stage B of the hoist-candidate funnel (LLM adjudication of Round 1's candidates).
3. **Adversarial validation** — Round 3 in [references/audit/VALIDATION-AND-REPORTING.md](${CLAUDE_PLUGIN_ROOT}/references/audit/VALIDATION-AND-REPORTING.md). Consolidate duplicates, then a separate agent tries to *disprove* each finding — targeting this domain's false-positive modes (incidental duplication, YAGNI, framework idioms).
4. **Structured output** — Round 4 in [references/audit/VALIDATION-AND-REPORTING.md](${CLAUDE_PLUGIN_ROOT}/references/audit/VALIDATION-AND-REPORTING.md), `${CLAUDE_PLUGIN_ROOT}/references/audit/report-schema.json`, `${CLAUDE_PLUGIN_ROOT}/references/audit/validate-findings.cjs`. Write and structurally validate `findings.json`.
5. **Independent fact-check** — Round 5 in [references/audit/VALIDATION-AND-REPORTING.md](${CLAUDE_PLUGIN_ROOT}/references/audit/VALIDATION-AND-REPORTING.md). A fresh agent per finding verifies only the *facts* (paths, line numbers, counts) — not the judgment.
6. **Human report** — Round 6 in [references/audit/VALIDATION-AND-REPORTING.md](${CLAUDE_PLUGIN_ROOT}/references/audit/VALIDATION-AND-REPORTING.md). Written last, so it never needs rewriting after verification. Each finding gets a remediation and a bucket: fix-now / tracked-debt / accepted.
